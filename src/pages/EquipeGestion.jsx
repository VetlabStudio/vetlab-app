...
export default function EquipeGestion() {
  ...
  async function revoquerMembre(memberId) {
    ...
    if (membre) {
      await supabase.from('profiles').update({ plan: 'gratuit' }).eq('id', membre.user_id)
    }
  ...
  async function changerRole(memberId, nouveauRole) {
    await supabase.from('team_members').update({ role: nouveauRole }).eq('id', memberId)
      ...
      <div className="calc-form">
        ...
        {equipe && (
          <div style={{ marginBottom: 24 }}>
            ...
            <p style={{ fontSize: 18, fontWeight: 700, color: 'var(--text-primary)' }}>{equipe.nom_clinique}</p>
        ...
        <div style={{ marginBottom: 24 }}>
          ...
          {msgSucces && (
            <div style={{
              ...
              background: '#E8F5E9', color: '#388E3C', fontSize: 13, fontWeight: 600,
            }}>
          ...
          <div style={{ marginBottom: 24 }}>
            ...
            {invitations.map(inv => (
              <div key={inv.id} style={{
                ...
                <button onClick={() => annulerInvitation(inv.id)} style={{
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                ...
                {m.role !== 'proprietaire' && m.user_id !== userId && (
                  <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
                    ...
                    <button onClick={() => setConfirmRevoquer(m.id)} style={{
                      background: 'none', border: '1px solid var(--accent-red)',
                      color: 'var(--accent-red)', borderRadius: 8, padding: '6px 10px',
